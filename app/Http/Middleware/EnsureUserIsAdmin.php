...
class EnsureUserIsAdmin
{
    ...
    public function handle(Request $request, Closure $next): Response
    {
        ...
        if ($utilisateur !== null)
        {
            ...
            if ($utilisateur->role()->where('role', 'like', 'Administrateur')->count() > 0)
            {
            ...
            return redirect('/')->with('alerte', 'Vous devez être authentifié avec un compte administrateur pour utiliser cette fonctionnalité.');
        }
        ...
        return redirect('/login')->with('alerte', 'Vous devez être connecté avec un compte pour avoir accès au site.');
    }
}
